...
import static com.watabou.pixeldungeon.PixelDungeon.groundTruthValues;
    ...
    public void run() {
        ...
        synchronized (methodStats) {
            ...
            if (!methodStats.isEmpty()) {
                ...
                for (com.watabou.MethodStat methodStat : methodStats) {
...
//            Log.d(TAG+" mainAppDb #4_1# ", "" + groundTruthValues.size());
        ...
        db.close();
        insert_locker.unlock();
        boolean ifcompress = com.watabou.Utils.checkfile(context);//get the size of db
        if(ifcompress) {//if the db is larger than limit size, compress it.
            com.watabou.Utils.compress(context);
        }
